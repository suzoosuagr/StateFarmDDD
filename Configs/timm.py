...
arg_lists = []
parser = argparse.ArgumentParser(description='timmParser')
...
# state farm dataset arguments
data = add_argument_group('StateFarm DDD Kaggle')
data.add_argument('--data_root', type=str, help='The root of dataset')
data.add_argument('--val_subjects', type=str, nargs="+", default=['p002','p012','p014'], help='valid subjects')
data.add_argument('--driver_image_csv', type=str, default='./data/driver_imgs_list.csv')
...
group.add_argument('--train-patience', type=int, default=30, metavar='N',
                    help='Training patience, ends train if patience.')
...
group.add_argument('--use_gpu', action='store_true', default=False, help='use gpu or not')

def get_config():
    configs, unparsed = parser.parse_known_args()
    return configs, unparsed

def read_textconfig(filepath):
...
def get_config_debug(filepath):
    content = read_textconfig(filepath)
    config, unparsed = parser.parse_known_args(content.split())
